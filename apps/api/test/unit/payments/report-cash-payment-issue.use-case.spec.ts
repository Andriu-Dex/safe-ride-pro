...
describe('ReportCashPaymentIssueUseCase', () => {
  it('fails when the payment does not exist', async () => {
    const repository = createPaymentsRepositoryMock();
    const useCase = new ReportCashPaymentIssueUseCase(repository);

    repository.findPaymentById.mockResolvedValue(null);

    await expect(
      useCase.execute('user-driver', 'missing', 'Pago no entregado por el pasajero'),
    ).rejects.toThrow(new NotFoundException('El pago no existe.'));
  });

  it('requires the trip driver, a cash payment and a meaningful note', async () => {
    const repository = createPaymentsRepositoryMock();
    const useCase = new ReportCashPaymentIssueUseCase(repository);

    repository.findPaymentById.mockResolvedValue(buildPayment());
    await expect(
      useCase.execute('other-user', 'payment-1', 'Pago no entregado'),
    ).rejects.toThrow(
      new ForbiddenException('Solo el conductor puede reportar este pago.'),
    );
    ...
    await expect(
      useCase.execute('user-driver', 'payment-1', 'Pago no entregado'),
    ).rejects.toThrow(
      new BadRequestException('Este pago no corresponde a efectivo.'),
    );

    repository.findPaymentById.mockResolvedValue(buildPayment());
    await expect(useCase.execute('user-driver', 'payment-1', '  corto  ')).rejects.toThrow(
      new BadRequestException('Describe brevemente la novedad del pago.'),
    );
  });

  it('registers the issue and notifies the passenger', async () => {
    const repository = createPaymentsRepositoryMock();
    const notifications = createNotificationsServiceMock();
    const useCase = new ReportCashPaymentIssueUseCase(
      repository,
      notifications as unknown as NotificationsService,
    );
    const updatedPayment = buildPayment({
      status: TripPaymentStatus.Failed,
      failureReason: 'El pasajero no realizo el pago acordado.',
    });

    repository.findPaymentById.mockResolvedValue(buildPayment());
    repository.markCashPaymentFailed.mockResolvedValue(updatedPayment);

    const response = await useCase.execute(
      'user-driver',
      'payment-1',
      '  El pasajero no realizo el pago acordado.  ',
    );

    expect(repository.markCashPaymentFailed).toHaveBeenCalledWith(
      'payment-1',
      'El pasajero no realizo el pago acordado.',
    );
    ...
    expect(response.message).toBe('Novedad de pago registrada.');
  });

  it('fails when the issue cannot be persisted', async () => {
    const repository = createPaymentsRepositoryMock();
    const useCase = new ReportCashPaymentIssueUseCase(repository);

    repository.findPaymentById.mockResolvedValue(buildPayment());
    repository.markCashPaymentFailed.mockResolvedValue(null);

    await expect(
      useCase.execute('user-driver', 'payment-1', 'El pasajero no realizo el pago.'),
    ).rejects.toThrow(new BadRequestException('No se pudo reportar la novedad.'));
  });
});
