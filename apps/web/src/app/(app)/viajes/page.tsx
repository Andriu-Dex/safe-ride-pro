...
import type { TripRecord } from '../../../modules/trips/types/trip';
import {
  acceptTripRequest,
  cancelTripRequest,
  createTripRequest,
...
export default function TripsPage() {
  ...
  const loadTripsData = async (accessToken: string) => {
    const [vehicleData, myTripsData, availableTripsData, myTripRequestsData, incomingTripRequestsData] = await Promise.all([
      ...
      listMyTrips(accessToken),
      listAvailableTrips(accessToken),
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        await loadTripsData(authSession.accessToken);
      } catch (error) {
        if (!isMounted) {
          return;
        }
  ...
  const handleTripAction = async (tripId: string, action: 'publish' | 'start' | 'complete' | 'cancel') => {
    if (!authSession) {
      return;
    }
  ...
  const handleRequestDraftChange = (tripId: string, field: keyof TripRequestDraft, value: string) => {
  ...
  const handleCreateRequest = async (trip: TripRecord) => {
    ...
    try {
      const response = await createTripRequest(authSession.accessToken, {
        ...
        requestedPickupLatitude: draft.requestedPickupLatitude ? Number.parseFloat(draft.requestedPickupLatitude) : undefined,
        requestedPickupLongitude: draft.requestedPickupLongitude ? Number.parseFloat(draft.requestedPickupLongitude) : undefined,
        requestedDropoffLatitude: draft.requestedDropoffLatitude ? Number.parseFloat(draft.requestedDropoffLatitude) : undefined,
        requestedDropoffLongitude: draft.requestedDropoffLongitude ? Number.parseFloat(draft.requestedDropoffLongitude) : undefined,
      });
  ...
  const handleIncomingRequestAction = async (requestId: string, action: 'accept' | 'reject') => {
    if (!authSession) {
      return;
    }
  ...
  const visibleAvailableTrips = availableTrips.filter((trip) => trip.driverMembershipId !== defaultMembershipId);
    ...
    return (
      <section className="loading-state compact-loading-state">
        <div className="loading-card">
          ...
          <p className="panel-text">Estamos cargando tus viajes, los cupos disponibles y las solicitudes vinculadas.</p>
        </div>
      </section>
    );
  }
      ...
      <section className="content-grid">
        ...
        <div className="page-grid page-grid-wide">
          ...
          <article className="panel panel-stack">
            ...
            {myTrips.length ? (
              <div className="list-stack">
                {myTrips.map((trip) => (
                  <div key={trip.id} className="list-card">
                    ...
                    <p className="panel-text">
                      {getTripRouteModeLabel(trip.routeMode)} · Cupos {trip.availableSeats}/{trip.seatCount} · Equipaje {getLuggagePolicyLabel(trip.luggagePolicySnapshot)}
                    </p>
                    <div className="button-row">
                      {trip.status === TripStatus.Draft ? (
                        <Button disabled={isMutatingTripId === trip.id} onClick={() => void handleTripAction(trip.id, 'publish')} variant="primary">
                          Publicar
                        </Button>
                      ) : null}
                      {(trip.status === TripStatus.Published || trip.status === TripStatus.Full) ? (
                        <Button disabled={isMutatingTripId === trip.id} onClick={() => void handleTripAction(trip.id, 'start')} variant="secondary">
                          Iniciar
                        </Button>
                      ) : null}
                      {trip.status === TripStatus.InProgress ? (
                        <Button disabled={isMutatingTripId === trip.id} onClick={() => void handleTripAction(trip.id, 'complete')} variant="secondary">
                          Finalizar
                        </Button>
                      ) : null}
                      {trip.status !== TripStatus.Completed && trip.status !== TripStatus.InProgress && trip.status !== TripStatus.Cancelled ? (
                        <Button disabled={isMutatingTripId === trip.id} onClick={() => void handleTripAction(trip.id, 'cancel')} variant="ghost">
                          Cancelar
                        </Button>
                      ) : null}
                    </div>
              ...
              <p className="panel-text">Aun no has creado viajes. El primero quedara en borrador.</p>
            )}
          </article>
        </div>
                ...
                {incomingRequests.map((request) => (
                  <div key={request.id} className="list-card">
                    <div className="list-card-header">
                      ...
                      <StatusPill label={getTripRequestStatusLabel(request.status)} tone={getTripRequestStatusTone(request.status)} />
                    </div>
                    <p className="panel-text">
                      Viaje: {request.tripOriginLabel} ? {request.tripDestinationLabel} · {formatDateTime(request.tripDepartureAt)}
                    </p>
                    {request.requestMessage ? <p className="panel-text">Mensaje: {request.requestMessage}</p> : null}
                    <div className="button-row">
                      {request.status === TripRequestStatus.Pending ? (
                        <>
                          <Button disabled={isMutatingRequestId === request.id} onClick={() => void handleIncomingRequestAction(request.id, 'accept')}>
                            Aceptar
                          </Button>
                          <Button disabled={isMutatingRequestId === request.id} onClick={() => void handleIncomingRequestAction(request.id, 'reject')} variant="secondary">
                            Rechazar
                          </Button>
                        </>
                      ) : null}
              ...
              <div className="list-stack">
                {myRequests.map((request) => (
                  <div key={request.id} className="list-card">
                    ...
                    {request.reviewNote ? <p className="panel-text">Revision: {request.reviewNote}</p> : null}
                    {(request.status === TripRequestStatus.Pending || request.status === TripRequestStatus.Accepted) ? (
                      <div className="button-row">
                        <Button disabled={isMutatingRequestId === request.id} onClick={() => void handleCancelMyRequest(request.id)} variant="ghost">
                          Cancelar solicitud
                        </Button>
                      </div>
                    ) : null}
        ...
        <article className="panel panel-stack">
          ...
          {visibleAvailableTrips.length ? (
              ...
              {visibleAvailableTrips.map((trip) => {
                ...
                return (
                  <div key={trip.id} className="list-card list-card-strong">
                    ...
                    <p className="panel-text">
                      Conductor: {trip.driverFullName} · {formatDateTime(trip.departureAt)}
                    </p>
                    <p className="panel-text">
                      {getTripRouteModeLabel(trip.routeMode)} · Cupos {trip.availableSeats}/{trip.seatCount} · {getVehicleTypeLabel(trip.vehicleTypeSnapshot)}
                    </p>
                    <p className="panel-text">
                      Precio base: ${trip.basePriceReference.toFixed(2)}
                      {trip.detourSurchargeReference ? ` · Desvio: $${trip.detourSurchargeReference.toFixed(2)}` : ''}
                    </p>
            ...
            <p className="panel-text">No hay viajes publicados por otros usuarios en este momento.</p>
          )}
        </article>
      </section>
    </>
